...
namespace IsometricMapViewer.Rendering
{
    public class GameRenderer : IDisposable
    {
        ...
        public void DrawMap(SpriteBatch spriteBatch, CameraHandler camera)
        {
            var visibleTiles = _map.GetVisibleTiles(camera.GetViewBounds());

            foreach (var tile in visibleTiles)
            {
                Vector2 pos = ToScreenCoordinates(tile.X, tile.Y);
                DrawSpriteIfExists(spriteBatch, tile.TileSprite, tile.TileFrame, pos, false);

                if (ShowObjects)
                    DrawSpriteIfExists(spriteBatch, tile.ObjectSprite, tile.ObjectFrame, pos, true);
            }
        }

        public void DrawGrid(SpriteBatch spriteBatch, CameraHandler camera)
        {
            if (ShowGrid)
            {
                _gridRenderer.Draw(spriteBatch, camera, true, true);
            }
        }

        public void DrawTileHighlight(SpriteBatch spriteBatch, CameraHandler camera, MapTile hoveredTile)
        {
            ...
            spriteBatch.Draw(_highlightTexture, highlightRect, null, Color.Yellow * 0.5f, 0f, Vector2.Zero, SpriteEffects.None, 0f);
        }

        public void DrawDebugOverlay(SpriteBatch spriteBatch, CameraHandler camera, MapTile hoveredTile, Vector2 mouseWorldPos)
        {
            using var debugRenderer = new DebugRenderer(spriteBatch, _font, spriteBatch.GraphicsDevice);
            debugRenderer.ShowHotkeys = this.ShowHotkeys;
            debugRenderer.Draw(spriteBatch, camera, hoveredTile, mouseWorldPos);
        }

        public void DrawThumbnails(SpriteBatch spriteBatch)
        {
            if (!ShowThumbnails) return;
            spriteBatch.Draw(_highlightTexture, ThumbnailPanelBounds, Color.Gray * 0.5f);
            int verticalStep = _thumbnailSize + _thumbnailSpacing;
            int visibleThumbnails = (int)Math.Ceiling((float)spriteBatch.GraphicsDevice.Viewport.Height / verticalStep) + 1;
            ...
            for (int i = _thumbnailScrollOffset; i < _thumbnailScrollOffset + visibleThumbnails && i < _sortedTextures.Count; i++)
            {
                ...
                spriteBatch.DrawString(_font, idText, textPosition, Color.White);
            }
        }
        ...
        private RenderTarget2D RenderMapToTexture(bool drawTiles, bool drawObjects)
        {
            ...
            _spriteBatch.Begin(
                ...
                Constants.PremultipliedBlendState,
            ...
            for (int y = 0; y < _map.Height; y++)
            {
                for (int x = 0; x < _map.Width; x++)
                {
                    ...
                    if (drawTiles) DrawSpriteIfExists(_spriteBatch, tile.TileSprite, tile.TileFrame, pos, false);
                    if (drawObjects) DrawSpriteIfExists(_spriteBatch, tile.ObjectSprite, tile.ObjectFrame, pos, true);
                }
            }
                               ...
                               bool drawObjects, RenderTarget2D renderTarget = null)
        {
            ...
            if (bounds.HasValue)
            {
                ...
                foreach (var tile in visibleTiles)
                {
                    ...
                    if (drawTiles)
                        DrawSpriteIfExists(_spriteBatch, tile.TileSprite, tile.TileFrame, pos, false);
                    if (drawObjects)
                        DrawSpriteIfExists(_spriteBatch, tile.ObjectSprite, tile.ObjectFrame, pos, true);
                }
            }
            else
            {
        ...
        private void DrawSpriteIfExists(SpriteBatch spriteBatch, int spriteId, int frameIndex,
                                        Vector2 position, bool isObjectSprite)
        {
            var texture = _spriteLoader.GetTexture(spriteId);
            if (texture == null) return;
            var frame = _spriteLoader.GetSpriteFrame(spriteId, frameIndex);
